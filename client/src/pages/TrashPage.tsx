...
import { Star, RotateCcw, Edit, Paperclip, Reply, Forward, PenLine } from 'lucide-react'
...
export default function TrashPage({ token, onViewEmail }: TrashPageProps) {
  ...
  const fetchTrashEmails = async () => {
    ...
    setError('')
    try {
      const response = await fetch('http://localhost:5050/api/delete', {
      ...
      setTrashEmails([])
      setError('Failed to load delete emails')
    } finally {
      setLoading(false)
    }
  }
