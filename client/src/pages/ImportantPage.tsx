...
import { Star, AlertCircle, Edit, Paperclip, Reply, Forward, PenLine } from 'lucide-react'
      ...
      {importantEmails.length > 0 && (
        <div className="email-list">
          {importantEmails.map((email, idx) => (
            <div
              key={idx}
              className={`email-item important-item ${!email.isRead && email.folder !== 'sent' && email.folder !== 'drafts' ? 'unread' : ''} ${email.folder === 'sent' ? 'sent' : ''}`}
