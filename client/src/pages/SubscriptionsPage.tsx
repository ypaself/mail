...
import { Star, LogOut, Edit, Paperclip, Reply, Forward, PenLine } from 'lucide-react'
      ...
      {subscriptionEmails.length > 0 && (
        <div className="email-list">
          {subscriptionEmails.map((email, idx) => (
            <div
              key={idx}
              className={`email-item subscription-item ${!email.isRead && email.folder !== 'sent' && email.folder !== 'drafts' ? 'unread' : ''} ${email.folder === 'sent' ? 'sent' : ''}`}
