...
import { Star, Archive, Edit, Paperclip, Reply, Forward, PenLine } from 'lucide-react'
      ...
      {archivedEmails.length > 0 && (
        <div className="email-list">
          {archivedEmails.map((email, idx) => (
            <div
              key={idx}
              className={`email-item archived-item ${!email.isRead && email.folder !== 'sent' && email.folder !== 'drafts' ? 'unread' : ''} ${email.folder === 'sent' ? 'sent' : ''}`}
